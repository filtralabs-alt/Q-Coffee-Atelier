...
export default function PlayQuiz() {
  ...
  const levels: { key: Level; label: string; color: string; stars: number }[] = [
    { key: "basic", label: t("play.quiz.basic"), color: "bg-green-500/10 text-green-600 dark:text-green-400", stars: 1 },
    { key: "intermediate", label: t("play.quiz.intermediate"), color: "bg-amber-500/10 text-amber-600 dark:text-amber-400", stars: 2 },
    { key: "advanced", label: t("play.quiz.advanced"), color: "bg-red-500/10 text-red-600 dark:text-red-400", stars: 3 },
  ];

  const getBadge = (pct: number) => {
    if (pct >= 90) return t("play.quiz.badge.master");
    if (pct >= 70) return t("play.quiz.badge.expert");
    if (pct >= 50) return t("play.quiz.badge.amateur");
    return t("play.quiz.badge.novice");
  };
  ...
  const handleNext = () => {
    ...
    if (currentQ + 1 >= questions.length) {
      ...
      const gameKey = `quiz-${selectedLevel}` as GameKey;
    ...
    return (
      <div className="flex flex-col min-h-full">
        ...
        <div className="px-5 pt-5 pb-4">
          <h1 className="font-serif text-xl font-semibold" data-testid="text-quiz-title">{t("play.quiz.title")}</h1>
          <p className="text-sm text-muted-foreground">{t("play.quiz.subtitle")}</p>
        </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">10 {t("play.quiz.questions")}</p>
                  </div>
                </div>
  ...
  if (finished) {
    ...
    const bonusText = { basic: t("play.quiz.bonus.basic"), intermediate: t("play.quiz.bonus.intermediate"), advanced: t("play.quiz.bonus.advanced") }[selectedLevel!];
    const bonus2Text = { basic: t("play.quiz.bonus2.basic"), intermediate: t("play.quiz.bonus2.intermediate"), advanced: t("play.quiz.bonus2.advanced") }[selectedLevel!];
    return (
      <div className="flex flex-col min-h-full">
        <GraosCelebration outcome={celebration} onDismiss={dismiss} />
        <div className="px-5 pt-4 pb-2">
          <Button variant="ghost" size="sm" onClick={reset} data-testid="button-quiz-back">
            <ArrowLeft className="h-4 w-4 mr-1" /> {t("play.quiz.back")}
          </Button>
        </div>
        <div className="flex-1 px-5 pb-4 flex flex-col items-center justify-center gap-3">
          <Card className="p-6 text-center max-w-sm mx-auto w-full" data-testid="card-quiz-result">
            <div className="space-y-4">
              ...
              <h2 className="font-serif text-2xl font-semibold">{t("play.quiz.score")}</h2>
              ...
              <Button onClick={reset} className="w-full mt-4" data-testid="button-quiz-retry">
                <RotateCcw className="h-4 w-4 mr-1.5" /> {t("play.quiz.retry")}
              </Button>
            </div>
          </Card>
          ...
          {!user && (
            <Card className="p-5 max-w-sm mx-auto w-full bg-primary/5 border-primary/20" data-testid="card-quiz-login-prompt">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <h3 className="font-serif text-sm font-semibold">{t("play.quiz.loginPrompt.title")}</h3>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed mb-3">{t("play.quiz.loginPrompt.desc")}</p>
          ...
          <Card className="p-5 max-w-sm mx-auto w-full" data-testid="card-quiz-bonus">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="secondary" className="text-[10px]">{t("play.quiz.bonus.label")}</Badge>
            </div>
            <h3 className="font-serif text-sm font-semibold mb-1.5">{t("play.quiz.bonus.title")}</h3>
            <p className="text-xs text-muted-foreground leading-relaxed" data-testid="text-quiz-bonus">{bonusText}</p>
            <p className="text-[10px] text-muted-foreground/70 mt-2">{t("play.quiz.bonus.source")}</p>
          </Card>

          <Card className="p-5 max-w-sm mx-auto w-full" data-testid="card-quiz-bonus2">
            <div className="flex items-center gap-2 mb-2">
              <Award className="h-4 w-4 text-primary" />
              <Badge variant="secondary" className="text-[10px]">{t("play.quiz.bonus2.label")}</Badge>
            </div>
            <h3 className="font-serif text-sm font-semibold mb-1.5">{t("play.quiz.bonus2.title")}</h3>
            <p className="text-xs text-muted-foreground leading-relaxed" data-testid="text-quiz-bonus2">{bonus2Text}</p>
            <p className="text-[10px] text-muted-foreground/70 mt-2">{t("play.quiz.bonus2.source")}</p>
          </Card>
        </div>
      </div>
    );
      ...
      <div className="px-5 pt-4 pb-2 space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-sm text-muted-foreground font-medium">
            {t("play.quiz.question")} {currentQ + 1} {t("play.quiz.of")} {questions.length}
          </span>
        </div>
      ...
      <div className="flex-1 px-5 space-y-3 pb-4">
          ...
          {selectedAnswer !== null && (
            <div className={`mt-4 p-3 rounded-md text-sm ${selectedAnswer === question.correctIndex ? "bg-green-500/10 text-green-700 dark:text-green-300" : "bg-red-500/10 text-red-700 dark:text-red-300"}`} data-testid="text-quiz-explanation">
              <p className="font-medium mb-1">
                {selectedAnswer === question.correctIndex ? t("play.quiz.correct") : t("play.quiz.incorrect")}
              </p>
        ...
        {selectedAnswer !== null && (
          <Button onClick={handleNext} className="w-full" data-testid="button-quiz-next">
            {currentQ + 1 >= questions.length ? t("play.quiz.finish") : t("play.quiz.next")}
