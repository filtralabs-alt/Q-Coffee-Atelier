...
const translations: Record<string, Record<Lang, string>> = {
  "app.name": { fr: "Cris Du Café", pt: "Cris Du Café" },
  ...
  "journal.evaluate": { fr: "Évaluer sur Q Coffee Go", pt: "Avaliar no Q Coffee Go" },
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [lang, setLangState] = useState<Lang>(() => {
    const stored = localStorage.getItem("cris-du-cafe-lang");
    return (stored === "pt" || stored === "fr") ? stored : "fr";
  });

  const setLang = useCallback((newLang: Lang) => {
    setLangState(newLang);
    localStorage.setItem("cris-du-cafe-lang", newLang);
  }, []);
